...
doing every build until it's done properly.

Thread::polling_page_offset() has become JavaThread::polling_page_offset()

AOT has been removed; cleared away a couple of remnants - remove
_call_aot_stub_size, and find_stub(/*is_aot*/ false) becomes just
find_stub()

Removed
#include "memory/metaspaceShared.hpp"
from thread_solaris_sparc.cpp, it's been renamed anyway and removed
from all the other os_cpu files.

The file src/hotspot/os_cpu/solaris_sparc/solaris_sparc.s has been
renamed to src/hotspot/os_cpu/solaris_sparc/solaris_sparc.S

Note: There's hunk in sparc-misc.patch for
src/hotspot/share/jvmci/vmStructs_jvmci.cpp regarding cpu features
that appears to have been reworked on other platforms, moved to the
enum Feature_Flag in vm_version_$CPU.hpp. But that's all implemented
quite differently between cpus and types of flags. Ignored for now.
